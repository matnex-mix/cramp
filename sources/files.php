...
class File {
	...
	public function dir_all( $path ){
		$contents = File::dir( $path, '', File::FILE_ONLY );
		$dirs = File::dir( $path, '', File::DIR_ONLY );
		foreach ($dirs as $dir) {
			$contents[$dir] = File::dir_all( $path.'/'.$dir );
		}
	...
	public function unlink_dir( $dirname ){
		if( is_dir($dirname) ){
